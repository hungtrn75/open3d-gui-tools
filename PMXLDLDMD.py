...
class Settings:
    ...
    def __init__(self):
        ...
        self.bg_color = gui.Color(209/255, 206/255, 206/255)
...
class AppWindow:
    ...
    def __init__(self, width, height):
        ...
        self.window = gui.Application.instance.create_window( "Phần mềm xử lý dữ liệu đám mây điểm", width, height)
        ...
        db_ctrls = gui.CollapsableVert("Lớp dữ liệu", 0.25 * em, gui.Margins(em, 0, 0, 0))
        ...
        self._fileedit_2 = gui.Checkbox("Dữ liệu gốc")
        self._fileedit_4 = gui.Checkbox("Dữ liệu giảm mật độ điểm")
        self._fileedit_3 = gui.Checkbox("Dữ liệu lọc thủ công")
        self._fileedit_5 = gui.Checkbox("Dữ liệu sau lọc thủ công")
        self._fileedit_6 = gui.Checkbox("Dữ liệu địa hình")
        self._fileedit_7 = gui.Checkbox("Dữ liệu phi địa hình")
        ...
        child_1 = gui.CollapsableVert("Dữ liệu gốc", 0.25 * em, gui.Margins(em, 0, 0, 0))
        child_2 = gui.CollapsableVert("Dữ liệu lọc tự động", 0.25 * em, gui.Margins(em, 0, 0, 0))
        child_3 = gui.CollapsableVert("Dữ liệu lọc thủ công", 0.25 * em, gui.Margins(em, 0, 0, 0))
        ...
        child_1.add_child(main_layout)
        child_1.add_child(downsample_layout)
        db_ctrls.add_child(child_1)
        
        child_2.add_child(ground_layout)
        child_2.add_child(non_ground_layout)
        db_ctrls.add_child(child_2)

        child_3.add_child(crop_layout)
        child_3.add_child(sub_layout)
        db_ctrls.add_child(child_3)

        db_ctrls.add_fixed(separation_height)
       
        self._settings_panel.add_child(db_ctrls)
        self._settings_panel.add_child(gui.Label(Settings.DIVIDER))
        ...
        if gui.Application.instance.menubar is None:
            ...
            file_menu.add_item("Giảm mật độ đám mây điểm", AppWindow.MENU_DOWNSAMPLING)
            file_menu.add_item("Lọc thủ công", AppWindow.MENU_CROP_GEOMETRY)
            file_menu.add_separator()
            file_menu.add_item("Lọc tự động địa hình", AppWindow.CSF_FILTER)
            file_menu.add_separator()
            ...
            if isMacOS:
                ...
                menu.add_menu("Menu tính năng", file_menu)
    ...
    def _on_menu_close_all(self):
        self.window.title =  "Phần mềm xử lý dữ liệu đám mây điểm"
    ...
    def _on_menu_crop_geometry(self):
        ...
        if c_geometry is not None:
            tmp = os.path.join(self.dir_path, "temp\\tmp.pcd") 
            s_tmp = os.path.join(self.dir_path,"crop_geometry.exe") 
            ...
            if return_code == 0:
                if out.splitlines()[0] == b'True':
                    save_path = os.path.join(self.dir_path,"temp\\c_geo.pcd") 
    ...
    def _on_menu_export_las(self):
        ...
        if len_true > 1:
            ...
            self.window.show_message_box("Chú ý", "Vui lòng chọn một PointCloud để xuất dữ liệu")
        else:
            dlg = gui.FileDialog(
    ...
    def _on_export_las_dialog_done(self, filename):
        ...
        if("workspace" not in filename):
            self._show_alert_dialog("Bạn vui lòng lưu file vào thư mục Workspace!")
        else:
    ...
    def _on_menu_csf_filter(self):
        ...
        self.relief_cb = gui.Checkbox("Địa hình đồi")
        self.relief_cb.set_on_checked(self._on_relief_cb)
        h4.add_child(self.relief_cb)
        self.flat_cb = gui.Checkbox("Đồng bằng")
    ...
    def _load_gui_on_separate_thread(self):
        if self._geometry is None:
            ...
            try:
                max = 1e8
                pynt_cloud, las, is_valid = PyntCloud.from_file_las(self._path, max)
                print(is_valid)
                if is_valid:
                    ...
                    self._show_alert_dialog("Số lượng điểm PointCloud không vượt quá {0} điểm".format(max))
            except Exception:
                pass
